...
class Publish extends Component {
	...
	onSubmit = e => {
		if (this.state.title && this.state.description && this.state.price) {
			axios
				.post(
					`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/offer/publish`,
					{
						...
						location: { 
							lat: this.state.lat, 
							lng: this.state.lng, 
							address: this.state.address || 'Brazzaville, Congo' 
						},
		...
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition((position) => {
				this.setState({
					...
					address: 'Position détectée (GPS)'
				});
			}, (error) => {
				alert("Impossible d'obtenir la position. Veuillez vérifier les permissions.");
			});
		} else {
			...
			<div className="container publish">
				...
				<form onSubmit={this.onSubmit}>
					...
					<select name="category" value={this.state.category} onChange={this.handleInputChange}>
						<option value="Électronique & Multimédia">Électronique & Multimédia</option>
						<option value="Véhicules">Véhicules</option>
						<option value="Immobilier">Immobilier</option>
						<option value="Mode & Vêtements">Mode & Vêtements</option>
						<option value="Maison & Meubles">Maison & Meubles</option>
					...
					<div className="location-picker">
						<input 
							...
							onChange={this.handleInputChange}
						/>
						<button type="button" className="geo-btn" onClick={this.getLocation}>
							<i className="fas fa-map-marker-alt" /> Ma position
						</button>
					</div>
					{this.state.lat && <span className="small success">Coordonnées GPS enregistrées !</span>}
